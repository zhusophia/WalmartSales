...
Impact of Variables on Weekly Sales 
  
- [Store and Sales](#store)
- [Month and Sales](#month)
- [Holidays and Sales](#holiday)
- [Unemployment Rate and Sales](#unemp) 
- [CPI and sales](#cpi)
- [Fuel Price and Sales](#fuel)
- [Temperature and Sales](#temp)
...
The conventional theory of the firm states that the principal objective of any business is to maximize profits. Internally, this is something the firm can attempt to control through increasing revenue or decreasing costs. However, since firms do not exist within a closed ecosystem, their profits are always at the mercy of the external environment. All firms can do is align their internal strategy with the external environmental conditions.  

Between January 2010 and December 2012, [the Walmart Sales data set](https://www.kaggle.com/datasets/mikhail1681/walmart-sales?resource=download) tracks weekly Walmart sales of 45 stores, alongside 5 external variables: Holiday_Flag, unemployment rate, CPI, fuel price, and temperature. This case study will compare Walmart’s experimental data with traditional literature to highlight the negligible impact of these 5 external variables on Walmart’s weekly sales. 

First, the data is prepared by rounding values and removing missing values. 
...
### Stores 20, 4, and 14 have the most amount of sales<a name="store"></a> 

First, weekly sales data is analyzed without taking the other variables(e.g. holidays, unemployment) into consideration.
...
However, there doesn't seem to be an easily identifiable correlation between sales and the variables (e.g. unemployment, temperature, etc.). Further analysis must be conducted to determine the relationship between these variables.  

### Sales peak in December and fall in January<a name="month"></a> 
...
### Super Bowl Week generates the most amount of sales<a name="holiday"></a>
...
holidaydate <- data %>%
  ...
  kable(caption = "Weeks including a holiday", holidaydate) %>% kable_styling()
```
...
holidayordering <- data %>% 
  ...
  kable(caption = "Holiday weeks with the most amount of sales", head(holidayordering)) %>% kable_styling()
...
### Stores 12, 28, and 38 have the highest unemployment rate<a name="unemp"></a>
...
### No correlation between CPI and average weekly sales<a name="cpi"></a> 
...
### No correlation between fuel price and average weekly sales <a name="fuel"></a>
...
### No correlation between temperature and average weekly sales<a name="temp"></a> 
...
### This dataset contains stores from 11 different countries<a name="geo"></a>
...
unempcountry <-  data.frame(Country = c("US", "Mexico", "Canada", "UK", "China", "Argentina", "Brazil", "Costa Rica", "El Salvador", "Guatemala", "Honduras", "Nicaragua", "India", "Chile"), Unemployment = c(9.61, 5.38, 8.175, 7.87, 4.5, 7.7, 8.4, 7.2, 4.9, 3.5, 4.1, 7.8, 5.5, 8.5))

unempcountry <- unempcountry %>%
  arrange(desc(Unemployment))

knitr::kable(unempcountry) %>%
  kable_styling(position = "center")  

```

This can be graphed like so: 
...
empbar <- data %>%
  ...
  mutate(Store = as.factor(Store), 
        geographic = cut(Unemployment, 
                          breaks = c(-Inf, 4.1, 4.5, 4.9, 5.38, 5.5, 7.2, 7.7, 7.8, 7.87, 8.17, 8.4, 8.5, 9.61, 10.5, Inf), 
                          labels = c("Guatemala", "Honduras", "China", "El Salvador", "Mexico", "India", "Costa Rica", "Argentina", "Nicaragua", "UK", "Canada", "Brazil",  "Chile", "US", "Unknown"), 
                          rights = TRUE)) %>%
  arrange(Unemployment)


pal <-  c("#004c91", "#007dc6", "#78b9e7", "#f47321", "#ffc220", "#367c2b","#76C143", "#222222", "#F2F8FD", "#004c91", "#007dc6", "blue", "purple", "yellow", "orange")
...
The plot highlights the different parts of the world each store may be located. It seems like the most data was obtained from Chile and India, while the least amount of data was obtained from Mexico, Argentina, and Nicaragua. 

### Sample Pearson correlation coefficient supports previous analysis<a name="stat"></a>

We can also use statistics to back up these claims by checking the correlation between weekly sales and other variables. This gives us insight into the relationship between each of the variables. 
...
- If $r_{X,Y}$ $\le$ $0$, the variables have a negative relationship. 
- If $r_{X,Y}$ $=$ $0$, the variables have no relationship. 
...
- Weekly sales and CPI: $r_{X,Y}$ $=$ $0$
- Weekly sales and fuel price: $r_{X,Y}$ $=$ $0$
- Weekly sales and temperature: $r_{X,Y}$ $=$ $0$ 
...
-- Replacing the y value (in this case, unemployment) with the other variables 
-- would yield the coefficient for that specific variable 
...
pctable <- data.frame(Variable = c("Unemployment", "CPI", "Fuel Prices", "Temperature"), "Correlation Coefficient" = c(-0.101436981393985, -0.0724959823775918, -0.00646895141525588,-0.0637688579328555))

colnames(pctable) <- c("Variable", "Correlation Coefficient")


  kable(caption = "Calculated pearson correlation coefficient values", pctable) %>% kable_styling()

```

### Point-Biserial correlation coefficient does not support previous analysis
...
colnames(pctable) <- c("Variable", "Correlation Coefficient")
  kable(caption = "Pearson and Point-Biserial Correlation Values", pctable) %>% kable_styling()

```

The correlation coefficient does not match with the data observed within the tables in charts, as there is a notable increase in sales due to holidays. 
...
There are many external variables that can impact Walmart sales. In particular, 

These conclusions were supported by the Pearson and Point-Biserial correlation coefficents. 

Potential extensions of this data could include.
