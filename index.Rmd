...
tempandsales <- data %>% 
  ...
  group_by(Temperature = round(Temperature, digits = 2)) %>%
  summarize(Weekly_Sales = mean(Weekly_Sales/1000000)) %>%
  arrange(desc(Weekly_Sales))
...
knitr::kable(data.frame(Date = c("2024/04", "2024/01", "2023/09", "2023/05", "2023/01", "2022/09","2022/05", "2022/01"),
             ...
             "Fuel Prices" =c(169.76, 145.9, 162.45, 160.62, 153.48, 154.09, 183.99, 153.88)), align="lcr") %>%
  kable_styling(position = "center")
...
In regard to this data set, the following relationships are found. 
